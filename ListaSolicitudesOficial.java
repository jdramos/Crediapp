...
import com.example.credimovil.modelo.SolicitudesOficialModel;
import com.example.credimovil.modelo.SolicitudesOficialesAdapter;
...
public class ListaSolicitudesOficial extends AppCompatActivity  {


    RequestQueue requestQueue;

    SharedPreferences preferencias;
    ListView lvSolicitudesOficial;
    ArrayList<SolicitudesModel> arrayList;
    SolicitudesOficialesAdapter adapter;
    ...
    double dblCantSolicitudes  = 0;
    double dblTotalSolicitado = 0;


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.solicitudes_oficiales_list_view);
        ...
        lvSolicitudesOficial = findViewById(R.id.listview_solicitudes_oficial);
        //textViewTotales = findViewById(R.id.btnDialog);
        arrayList = new ArrayList<>();
        adapter = new SolicitudesOficialesAdapter(this, arrayList);
        tvSucursal = findViewById(R.id.tvNomOfi);
        DecimalFormat precision = new DecimalFormat("#,##0.00");

        this.setTitle("Solicitudes por oficiales");
        ...
        url = spWeb+"/"+spEmpresa+"/listar_solicitudes_oficial.php?numSuc="+strNumSuc+"&codOfi="+spCodOfi+"&rol="+rol;

        espere();
        saldos(url);

        lvSolicitudesOficial.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                SolicitudesModel sm = (SolicitudesModel) adapter.getItem(position);
                Intent i = new Intent(getApplicationContext(), ListaSolicitudes.class);
                i.putExtra("numSuc", strNumSuc);
                i.putExtra("nomOfi", sm.getNomCli());
                i.putExtra("codOfi", sm.getCodOfi());
                i.putExtra("aprobado", sm.getAprobado());
                i.putExtra("rol", rol);
            ...
            public void onResponse(JSONArray response) {
                ...
                try {
                    for(int i =0;i < response.length(); i++){
                        ...
                        String valorTotal    = (jsonObject.getString("monto"));
                        String estado    = (jsonObject.getString("estado"));

                        arrayList.add(new SolicitudesModel(codOfi, nomOfi, Integer.valueOf(cantSolicitudes), valorTotal, estado ));
                        adapter = new SolicitudesOficialesAdapter(getApplicationContext(), arrayList);
                        lvSolicitudesOficial.setAdapter(adapter);

                        dblCantSolicitudes = dblCantSolicitudes + Double.parseDouble(cantSolicitudes);
                        dblTotalSolicitado = dblTotalSolicitado + Double.parseDouble(valorTotal);
                    }
                progreso.dismiss();
    ...
    private void showDialog() {
        ...
        txtTotalClientes.setText(precision.format(dblCantSolicitudes));
        tvTotalClientes.setText("Cantidad solicitudes");
        txtTotalSaldo.setText(precision.format(dblTotalSolicitado));
        tvTotalSaldo.setText("Total solicitado C$");
    ...
    public void filtrar(String texto) {
        ...
        for(SolicitudesModel solicitudes : arrayList) {
            if(solicitudes.getCodOfi().toLowerCase().contains(texto.toLowerCase())
                    || solicitudes.getNomOfi().toLowerCase().contains(texto.toLowerCase())) {
